...
class ParallelLayer(nn.Module):
    ...
    def run_blocks(self, flat_input: torch.Tensor) -> torch.Tensor:
        ...
        if torch.isnan(flat_input).any():
            raise ValueError(f"[ERROR][ParallelLayer] NaN detected in flat_input!")
        if torch.isinf(flat_input).any():
            raise ValueError(f"[ERROR][ParallelLayer] Inf detected in flat_input!")
        ...
        for block_name, block in self._parallel_blocks.items():
            ...
            if torch.isnan(output).any():
                raise ValueError(f"[ERROR][ParallelLayer] NaN detected in output of block: {block_name}")
            if torch.isinf(output).any():
                raise ValueError(f"[ERROR][ParallelLayer] Inf detected in output of block: {block_name}")
            blocks_outputs[self._output_component_name_prefix + block_name] = output
