...
class MoeLayer(nn.Module):
    ...
    def forward(self, x):
        if torch.isnan(x).any():
            raise ValueError("[ERROR][MoeLayer] NaN detected in input x!")
        if torch.isinf(x).any():
            raise ValueError("[ERROR][MoeLayer] Inf detected in input x!")

        gate_output = self.gate(x)
        if torch.isnan(gate_output).any():
            raise ValueError("[ERROR][MoeLayer] NaN detected in gate_output!")
        
        gate_scores = F.softmax(gate_output, dim=-1)  # [batch, num_experts] # gate the expert outputs
        if torch.isnan(gate_scores).any():
            raise ValueError("[ERROR][MoeLayer] NaN detected in gate_scores!")

        expert_outputs = []
        for i, expert in enumerate(self.experts):
            out = expert(x)
            if torch.isnan(out).any():
                raise ValueError(f"[ERROR][MoeLayer] NaN detected in expert_{i} output!")
        ...
        if torch.isnan(output).any():
            raise ValueError("[ERROR][MoeLayer] NaN detected in final output!")
